...
async def export_folder_comparison_xlsx(folder_id: str, request: FolderComparisonExportRequest):
    ...
    # Sheet 5: Normalized to Baseline - Spontaneous Activity
    ws_norm_spont = wb.create_sheet("Normalized Spontaneous")
    ...
    # Section title
    ws_norm_spont.merge_cells('A1:I1')
    ws_norm_spont['A1'] = 'Normalized to Baseline — Spontaneous Activity'
    ws_norm_spont['A1'].font = Font(bold=True, size=11)
    ws_norm_spont['A1'].fill = PatternFill(start_color="374151", end_color="374151", fill_type="solid")
    ws_norm_spont['A1'].font = Font(bold=True, size=11, color="FFFFFF")
    ws_norm_spont['A1'].alignment = Alignment(horizontal='center')
    ...
    for col_idx, (header, header_type) in enumerate(norm_spont_headers, start=1):
        cell = ws_norm_spont.cell(row=2, column=col_idx, value=header)
        cell.font = header_font
        cell.border = thin_border
        cell.alignment = Alignment(horizontal='center', wrap_text=True)
        if header_type == 'baseline':
    ...
    for row_idx, rec in enumerate(recordings, start=3):
        ws_norm_spont.cell(row=row_idx, column=1, value=rec.get('name', '')).font = data_font
        ...
        ws_norm_spont.cell(row=row_idx, column=2, value=format_value(n_baseline_bf, 1)).font = data_font
        ws_norm_spont.cell(row=row_idx, column=3, value=format_value(n_baseline_ln_rmssd, 1)).font = data_font
        ws_norm_spont.cell(row=row_idx, column=4, value=format_value(n_baseline_ln_sdnn, 1)).font = data_font
        ws_norm_spont.cell(row=row_idx, column=5, value=format_value(n_baseline_pnn50, 1)).font = data_font
        ws_norm_spont.cell(row=row_idx, column=6, value=format_value(n_drug_bf, 1)).font = data_font
        ws_norm_spont.cell(row=row_idx, column=7, value=format_value(n_drug_ln_rmssd, 1)).font = data_font
        ws_norm_spont.cell(row=row_idx, column=8, value=format_value(n_drug_ln_sdnn, 1)).font = data_font
        ws_norm_spont.cell(row=row_idx, column=9, value=format_value(n_drug_pnn50, 1)).font = data_font
        ...
        for col in range(1, 10):
            ws_norm_spont.cell(row=row_idx, column=col).border = thin_border
            if 2 <= col <= 5:
                ws_norm_spont.cell(row=row_idx, column=col).fill = PatternFill(start_color="FEF3C7", end_color="FEF3C7", fill_type="solid")
            elif 6 <= col <= 9:
                ws_norm_spont.cell(row=row_idx, column=col).fill = PatternFill(start_color="EDE9FE", end_color="EDE9FE", fill_type="solid")
    
    # Folder Average row
    norm_avg_row = len(recordings) + 3
    ws_norm_spont.cell(row=norm_avg_row, column=1, value=f"Folder Average (n={len(recordings)})").font = Font(bold=True, size=9)
    ...
    for col_idx, key in enumerate(['baseline_bf', 'baseline_ln_rmssd', 'baseline_ln_sdnn', 'baseline_pnn50',
        ...
        avg_val = sum(norm_sums[key]) / len(norm_sums[key]) if norm_sums[key] else None
        ws_norm_spont.cell(row=norm_avg_row, column=col_idx, value=format_value(avg_val, 1)).font = Font(bold=True, size=9)
    
    for col in range(1, 10):
        ws_norm_spont.cell(row=norm_avg_row, column=col).fill = avg_fill
        ws_norm_spont.cell(row=norm_avg_row, column=col).border = thin_border
    
    auto_width(ws_norm_spont)
    
    # Sheet 6: Normalized to Baseline - Light HRA
    ws_norm_light = wb.create_sheet("Normalized Light HRA")
    
    # Section title
    ws_norm_light.merge_cells('A1:E1')
    ws_norm_light['A1'] = 'Normalized to Baseline — Light-Induced Heart Rate Adaptation (HRA)'
    ws_norm_light['A1'].font = Font(bold=True, size=11, color="FFFFFF")
    ws_norm_light['A1'].fill = header_fill_light
    ws_norm_light['A1'].alignment = Alignment(horizontal='center')
    ...
    for col_idx, header in enumerate(norm_light_headers, start=1):
        cell = ws_norm_light.cell(row=2, column=col_idx, value=header)
        cell.font = header_font
        cell.fill = header_fill_light
        cell.border = thin_border
        cell.alignment = Alignment(horizontal='center', wrap_text=True)
    
    # Data rows with normalized values (using same avg_baseline_bf from spontaneous)
    light_norm_sums = {'baseline_bf': [], 'avg_bf': [], 'peak_bf': [], 'recovery_bf': []}
    
    for row_idx, rec in enumerate(recordings, start=3):
        ws_norm_light.cell(row=row_idx, column=1, value=rec.get('name', '')).font = data_font
        ...
        ws_norm_light.cell(row=row_idx, column=2, value=format_value(n_light_baseline_bf, 1)).font = data_font
        ws_norm_light.cell(row=row_idx, column=3, value=format_value(n_light_avg_bf, 1)).font = data_font
        ws_norm_light.cell(row=row_idx, column=4, value=format_value(n_light_peak_bf, 1)).font = data_font
        ws_norm_light.cell(row=row_idx, column=5, value=format_value(n_light_recovery_bf, 1)).font = data_font
        ...
        for col in range(1, 6):
            ws_norm_light.cell(row=row_idx, column=col).border = thin_border
    
    # Folder Average row
    light_norm_avg_row = len(recordings) + 3
    ws_norm_light.cell(row=light_norm_avg_row, column=1, value=f"Folder Average (n={len(recordings)})").font = Font(bold=True, size=9)
    
    for col_idx, key in enumerate(['baseline_bf', 'avg_bf', 'peak_bf', 'recovery_bf'], start=2):
        avg_val = sum(light_norm_sums[key]) / len(light_norm_sums[key]) if light_norm_sums[key] else None
        ws_norm_light.cell(row=light_norm_avg_row, column=col_idx, value=format_value(avg_val, 1)).font = Font(bold=True, size=9)
    
    for col in range(1, 6):
        ws_norm_light.cell(row=light_norm_avg_row, column=col).fill = avg_fill
        ws_norm_light.cell(row=light_norm_avg_row, column=col).border = thin_border
    
    auto_width(ws_norm_light)
...
async def export_folder_comparison_pdf(folder_id: str, request: FolderComparisonExportRequest):
    ...
    with PdfPages(output) as pdf:
        ...
        # Page 5: Normalized to Baseline
        fig5 = plt.figure(figsize=(11, 8.5))
        
        # Main title
        fig5.text(0.5, 0.95, 'Normalized to Baseline', fontsize=14, fontweight='bold', ha='center')
        ...
        # Spontaneous Activity Normalized Section
        ax_norm_spont = fig5.add_axes([0.05, 0.50, 0.90, 0.40])
        ax_norm_spont.axis('off')
        
        fig5.text(0.5, 0.88, 'Spontaneous Activity (% of Baseline)', fontsize=11, fontweight='bold', 
                 color='#374151', ha='center')
        
        norm_spont_headers = ['Recording', 'Base BF%', 'Base ln(RMSSD)%', 'Base ln(SDNN)%', 'Base pNN50%',
                             'Drug BF%', 'Drug ln(RMSSD)%', 'Drug ln(SDNN)%', 'Drug pNN50%']
        ...
        for rec in recordings:
            ...
            norm_spont_data.append([
                rec.get('name', '')[:12],
                fmt(n_baseline_bf, 1), fmt(n_baseline_ln_rmssd, 1), fmt(n_baseline_ln_sdnn, 1), fmt(n_baseline_pnn50, 1),
                fmt(n_drug_bf, 1), fmt(n_drug_ln_rmssd, 1), fmt(n_drug_ln_sdnn, 1), fmt(n_drug_pnn50, 1),
            ])
        ...
        norm_spont_data.append([
            f'Avg (n={len(recordings)})',
            fmt(sum(norm_sums['baseline_bf'])/len(norm_sums['baseline_bf']) if norm_sums['baseline_bf'] else None, 1),
            fmt(sum(norm_sums['baseline_ln_rmssd'])/len(norm_sums['baseline_ln_rmssd']) if norm_sums['baseline_ln_rmssd'] else None, 1),
            fmt(sum(norm_sums['baseline_ln_sdnn'])/len(norm_sums['baseline_ln_sdnn']) if norm_sums['baseline_ln_sdnn'] else None, 1),
            fmt(sum(norm_sums['baseline_pnn50'])/len(norm_sums['baseline_pnn50']) if norm_sums['baseline_pnn50'] else None, 1),
            fmt(sum(norm_sums['drug_bf'])/len(norm_sums['drug_bf']) if norm_sums['drug_bf'] else None, 1),
            fmt(sum(norm_sums['drug_ln_rmssd'])/len(norm_sums['drug_ln_rmssd']) if norm_sums['drug_ln_rmssd'] else None, 1),
            fmt(sum(norm_sums['drug_ln_sdnn'])/len(norm_sums['drug_ln_sdnn']) if norm_sums['drug_ln_sdnn'] else None, 1),
            fmt(sum(norm_sums['drug_pnn50'])/len(norm_sums['drug_pnn50']) if norm_sums['drug_pnn50'] else None, 1),
        ])
        
        n_norm_spont_rows = len(norm_spont_data)
        norm_spont_row_height = min(0.05, 0.30 / n_norm_spont_rows)
        norm_spont_table_height = norm_spont_row_height * n_norm_spont_rows
        
        table_norm_spont = ax_norm_spont.table(cellText=norm_spont_data, loc='upper center', cellLoc='center',
                                               colWidths=[0.13, 0.10, 0.11, 0.10, 0.10, 0.10, 0.11, 0.10, 0.10],
                                               bbox=[0.02, 1.0 - norm_spont_table_height - 0.08, 0.96, norm_spont_table_height])
        table_norm_spont.auto_set_font_size(False)
        table_norm_spont.set_fontsize(7)
        
        for (row, col), cell in table_norm_spont.get_celld().items():
            cell.set_edgecolor('#d0d0d0')
            if row == 0:
                if 1 <= col <= 4:  # Baseline columns
                    cell.set_facecolor('#F59E0B')
                elif 5 <= col <= 8:  # Drug columns
        ...
        # Light HRA Normalized Section
        ax_norm_light = fig5.add_axes([0.15, 0.05, 0.70, 0.38])
        ax_norm_light.axis('off')
        
        fig5.text(0.5, 0.42, 'Light-Induced Heart Rate Adaptation (% of Baseline)', fontsize=11, fontweight='bold', 
                 color='#374151', ha='center')
        
        norm_light_headers = ['Recording', 'Baseline BF%', 'Avg BF%', 'Peak BF%', 'Recovery BF%']
        norm_light_data = [norm_light_headers]
        ...
        norm_light_data.append([
            f'Folder Average (n={len(recordings)})',
            fmt(sum(light_norm_sums['baseline_bf'])/len(light_norm_sums['baseline_bf']) if light_norm_sums['baseline_bf'] else None, 1),
            fmt(sum(light_norm_sums['avg_bf'])/len(light_norm_sums['avg_bf']) if light_norm_sums['avg_bf'] else None, 1),
            fmt(sum(light_norm_sums['peak_bf'])/len(light_norm_sums['peak_bf']) if light_norm_sums['peak_bf'] else None, 1),
            fmt(sum(light_norm_sums['recovery_bf'])/len(light_norm_sums['recovery_bf']) if light_norm_sums['recovery_bf'] else None, 1),
        ])
        
        n_norm_light_rows = len(norm_light_data)
        norm_light_row_height = min(0.05, 0.28 / n_norm_light_rows)
        norm_light_table_height = norm_light_row_height * n_norm_light_rows
        
        table_norm_light = ax_norm_light.table(cellText=norm_light_data, loc='upper center', cellLoc='center',
                                               colWidths=[0.30, 0.17, 0.17, 0.17, 0.17],
                                               bbox=[0.02, 1.0 - norm_light_table_height - 0.10, 0.96, norm_light_table_height])
        table_norm_light.auto_set_font_size(False)
        table_norm_light.set_fontsize(8)
        
        for (row, col), cell in table_norm_light.get_celld().items():
            cell.set_edgecolor('#d0d0d0')
            if row == 0:
                cell.set_facecolor('#06B6D4')
                cell.set_text_props(color='white', fontweight='bold')
            elif row == len(norm_light_data) - 1:
                cell.set_facecolor('#E5E7EB')
                cell.set_text_props(fontweight='bold')
